...
G4VPhysicalVolume* ICRP110PhantomConstruction::Construct()
{
    ...
    //G4VPhysicalVolume* motherVolume = new G4PVPlacement(0,G4ThreeVector(),
          ...
          //				                                         false,
          //				                                         0);
    G4VPhysicalVolume* motherVolume = detector->Construct();
    G4LogicalVolume* logicWorld = motherVolume->GetLogicalVolume();
    this->copyConstruct(detector);
    ...
    //detector->UpdateWorldDataWithPhantom(motherVolume);
    paramPV = *param;
    return motherVolume;
}

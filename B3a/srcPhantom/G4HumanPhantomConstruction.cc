...
G4HumanPhantomConstruction::G4HumanPhantomConstruction(DetectorConstruction* det):det(det)
{
...
G4VPhysicalVolume* G4HumanPhantomConstruction::Construct()
{
  ...
  G4VPhysicalVolume* wrld = det->Construct();
  this->copyConstruct(det);
  std::cout << det->EJ208 << " :EJ208-det" << std::endl;
  std::cout << this->EJ208 << " :EJ208-phantom" << std::endl;
  std::cout << det->MPT_UPDATE << " :MPT-det" << std::endl;
  std::cout << this->MPT_UPDATE << " :MPT-phantom" << std::endl;
  builder->SetMotherVolume(wrld);
  //builder->SetMotherVolume(worldVolume);
  ...
  delete builder;
  //det->UpdateWorldDataWithPhantom(result);
  return result; 
}
